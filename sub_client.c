...
void my_message_callback(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message)
{
	...
	if( strncmp( message->topic, "com/pc3",6)==0){
                printf("PC Command === message %s\n",message->payload);
                //Parse and send message to enable the camera streaming
                int command =-1;
		int t=-999;
		char *ptr = NULL;
		char payload[512];
		strcpy(payload, message->payload);
                if( ptr=strstr(message->payload,"temp")!=NULL){
                        command =1;//ON
                }else{
                        command=0;//OFF
                }
		if(ptr){
		printf("%s",payload+4);
		//printf("PTR %u, %u, %d\n",ptr,message->payload, message->payload-ptr);
			t=atoi(payload+4);
		}	
                printf("Command = %d, temp=%d\n",command, t);
                //send message to your service
                if( command !=-1 && t!=-999){
                        //MESSAGE to service
			printf("MSG to service temp=%d\n",t);
			char command[256];

			sprintf(command, "python /home/ekagga/script/wrDB.py --temp %d",t);
			system(command);
                }


        }
        if( strncmp( message->topic, "com/sp",6)==0){
                printf("Smart Plug commad");
                //Parse and send message to enable smart plug
